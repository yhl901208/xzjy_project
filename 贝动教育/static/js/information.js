$(function () {
    ...
    uploader_fan.on('uploadSuccess', function (file, response) {
      ...
      //技能证书
    var uploader_jn = WebUploader.create({
        // 文件接收服务端。
        server: "/wechat/upload/index.html",
        // 内部根据当前运行是创建，可能是input元素，也可能是flash.
        pick: '.upload-jn',
     ...
     //文件上传进度
    uploader_jn.on('uploadProgress', function (file, percentage) {

    });
    //文件上传进度
    uploader_jn.on('uploadSuccess', function (file, response) {
